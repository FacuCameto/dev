...
if (ControlName == "First Name" || RunAll) {
    if (VV.Form.Global.CentralValidation(VV.Form.GetFieldValue("First Name"), "Blank") == false) {
        VV.Form.SetValidationErrorMessageOnField("First Name", "A value needs to be entered.");
...
if (ControlName == "Last Name" || RunAll) {
    if (VV.Form.Global.CentralValidation(VV.Form.GetFieldValue("Last Name"), "Blank") == false) {
        VV.Form.SetValidationErrorMessageOnField("Last Name", "A value needs to be entered.");
        ErrorReporting = false;
    } else {
...
if (ControlName == "Role Selector" || RunAll) {
    if (VV.Form.Global.CentralValidation(VV.Form.GetFieldValue("Role Selector"), "DDSelect") == false) {
        VV.Form.SetValidationErrorMessageOnField("Role Selector", "A value needs to be selected.");
        ErrorReporting = false;
    } else {
        VV.Form.ClearValidationErrorOnField("Role Selector");
    }
}
